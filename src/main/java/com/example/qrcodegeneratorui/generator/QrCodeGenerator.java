...
public class QrCodeGenerator {
     ...
     */
    private final int errCorrLevel;
     ...
     * [][(version - 1) * 4 + Error Correction Level], i.e. 3Q nm (version 3, error correcion M) = [0][2 * 4 + 1]
     */
    ...
    public QrCodeGenerator(String input, String errCorrLevel)
    {
        ...
        this.input = input;
        this.errCorrLevel = switch(errCorrLevel){
            case "LOW" -> 0;
            case "MEDIUM" -> 1;
            case "QUARTILE" -> 2;
            case "HIGH" -> 3;
            default -> throw new IllegalStateException("Unexpected value: " + errCorrLevel);
        };
        ...
        fitToVersionNumber();
        addPaddingBytes();
        ErrorCorrectionCode ecc = new ErrorCorrectionCode(versionNumber, this.errCorrLevel, encodedInput);
        draw = new DrawQR(versionNumber, ecc.getFinalMessage(), this.errCorrLevel);
    }
